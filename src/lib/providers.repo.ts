...
const DEFAULT_PROVIDERS: NewProviderInput[] = [
  ...
  // Gemini via its OpenAI-compatible surface (/v1beta/openai) so it works with
  // the same "Authorization: Bearer" flow as every other LLM provider and can
  // be mixed into Combos AI. The native /v1beta ...:generateContent API uses a
  // different auth (query "key") and body shape — not used here.
  { slug: 'gemini', name: 'Google Gemini', category: 'llm', isLlm: true, isFree: true, rotationStrategy: 'priority', defaultInjectLocation: 'header', defaultInjectKeyName: 'Authorization', defaultInjectValueTemplate: 'Bearer {key}', defaultBaseUrl: 'https://generativelanguage.googleapis.com/v1beta/openai', models: ['gemini-2.5-flash', 'gemini-2.5-flash-lite', 'gemini-2.5-pro'] },
